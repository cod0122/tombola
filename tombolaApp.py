...
class BufferNumeri:
    ...
    def _resetBuffer(self):
        self._BNumeri = list(self._BNumeri_empty)
        self._idxBufferInp=-1
        self._idxBufferOut=-1
    ...
    def _comprimeBuffer(self):    
        ...
        _lBAppoggio = self._BNumeri[self._idxBufferOut:self._idxBufferInp] 
...
class thSerialPort (threading.Thread):
    ...
    def _onGetDaRS232(self):
        ...
        try:
            ...
            _BufferInput = self._iserial.read(1)
            #ilog.stampa('Letto da RS232: {0}'.format(_BufferInput))

            #--- da RS232 della Tombola mi aspetto un byte che devo convertire nel numero
            if _BufferInput[:1] > b'':
                #_numero = ord(_BufferInput[:1])
                _numeroX = str(ord(_BufferInput[:1]))
                if not (_numeroX.isdigit()):
                    ilog.stampa('ERRORE letto da RS232 dato NON numerico: {0}'.format(_BufferInput))
                    print ('ERRORE DATO DA RS232 NON NUMERICO: {0}'.format(_BufferInput))
                else:
                    _numero = int(_numeroX)
                    self._ultimoNumero = _numero
                    #if _numero > 200:
                    #    ilog.stampa('letto da RS232 il carattere speciale: {0}'.format(_numeroX))
                    #    #print ('letto da RS232 il carattere speciale: {0}'.format(_numeroX))
                    #    self._ultimoNumero=0
                    #else:
                    #    if (_numero - self._ultimoNumero) > 1:
                    #        ilog.stampa('ERRORE letto da RS232 dato NON in sequenza da {0} salta a {1}'.format(self._ultimoNumero, _numero))
                    #        print ('ERRORE letto da RS232 dato NON in sequenza da {0} salta a {1}'.format(self._ultimoNumero, _numero))
                    #        self._ultimoNumero = _numero
                    #    else:
                    #        self._ultimoNumero = _numero
                
                    #Inserisce numero nel BUFFER di lavoro 
                    iBufferNumeri._addNumero(_numero)   
  ...
  #          print ('Errore risorsa GPIO (4) non disponibile')
            ...
            #print ('_onGetDaRS232: Errore Input senza dati. Proseguo comunque')
...
class App:
    ...
    def _onInizializza(self):
        ...
        try:
            self._SCREEN = pygame.display.set_mode(_size, _flags, _depth) 
            
        except (pygame.error):
            print ('Risoluzione  {0}  non supportata dal Video'.format (_size))
            print (pygame.get_error())
            #self._SCREEN = pygame.display.set_mode(_sizeExtra, _flags, _depth) 
            raise (SystemExit)
        
        ilog.stampa('Risoluzione accettata self._SCREEN: ', [self._SCREEN])
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) > 2: 
        ...
        print('ESEMPIO: sudo python tombolaApp e 1660,1200 f f')
